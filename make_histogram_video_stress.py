"""
Same as make_histogram_video_solution.py but bins the pk1_stess_3 field
instead - this one is a stress state at each frame, not something
cumulative, so what's being binned is "current stress direction" rather
than "direction traveled so far".

Plain (non-expanded) segmentation mask - see the _expanded_mask twin.
"""
import os
import numpy as np
...
FRAME_DIR = f"{RESULT_DIR}/angle_histograms_pk1_stess_3"
VIDEO_OUT = f"{RESULT_DIR}/angle_histograms_pk1_stess_3.mp4"
FIELD = "pk1_stess_3"
EXPANDED = False
N_BINS = 24
...
for t in frames:
    ...
    fig.suptitle(f"pk1_stess_3 field, t={t} - current stress direction, migration frame")
    fig.tight_layout()
