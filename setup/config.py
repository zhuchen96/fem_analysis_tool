"""
Every number in this file is specific to one imaging session / one embryo -
nothing here can be figured out automatically, someone has to look at the
data and decide. If you're setting this pipeline up for a brand new
sequence, this is the only file you should need to edit before running
01 through 06 (see setup/README section in the main README for the full
walkthrough, and pick_seed_points.py for help with the two values below
that need you to actually look at an image).
"""

# micron per pixel, straight from the microscope / acquisition metadata.
# groups.py (one level up) has its own copy of this same number for the main
...
# two, all the way back toward frame 1. Their midpoint becomes the "midline"
# that separates neuromast from reference tissue; the second point alone
# becomes the "apical point" that further splits the neuromast into a front
